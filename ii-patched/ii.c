...
static void usage() {
	fputs("ii - irc it - " VERSION "\n"
	      ...
	      "usage: ii [-i <irc dir>] [-s <host>] [-p <port>] [-u <username>]\n"
...
static void login(char *key, char *fullname, char *username) {
	if(key) snprintf(message, PIPE_BUF,
				"PASS %s\r\nNICK %s\r\nUSER %s localhost %s :%s\r\n", key,
				nick, username ? username : nick, host, fullname ? fullname : nick);
